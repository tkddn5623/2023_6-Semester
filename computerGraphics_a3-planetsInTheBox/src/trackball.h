...
struct trackball
{
	...
	int		button = 0;				// Mouse control
	int		mods = 0;				// Mouse control
	float	scale_zoom = 300.0f;	// Mouse control
	...
	camera update_pan(vec2 m) const;
	camera update_zoom(vec2 m) const;
};
...
inline camera trackball::update_pan(vec2 m) const
{
	vec3 p1 = vec3(m - m0, 0); // displacement
	...
	vec3 u = c.up.cross(n).normalize();
	vec3 v = n.cross(u).normalize();

	c.eye = c.eye - (u * p1.x + v * p1.y);	
	c.at = c.at - (u * p1.x + v * p1.y);
	c.view_matrix = mat4::look_at(c.eye, c.at, c.up);

	return c;
}

inline camera trackball::update_zoom(vec2 m) const
{
	vec3 p1 = vec3(m - m0, 0); // displacement
