...
//*************************************
// global constants
static const char* window_name =      "Assignment 1: Moving Circles";
static const char* vert_shader_path = "shaders/circ.vert";
static const char* frag_shader_path = "shaders/circ.frag";
static const uint  MIN_TESS = 100;       // minimum tessellation factor (down to a triangle)
...
static const int   MIN_CIRCLES = 16;     // A1: Min number of circles
static const int   MAX_CIRCLES = 32;     // A1: Max number of circles
//*************************************
// window objects
GLFWwindow* window = nullptr;
ivec2       window_size = ivec2(1280, 720); // initial window size (16/9)
...
#endif
int     num_of_circles = 20;             // A1: Number of circles
auto    circles = std::move(create_circles(num_of_circles, t));
...
void update()
{
	...
	GLint uloc;
	if ((uloc = glGetUniformLocation(program, "b_solid_color")) > -1) glUniform1i(uloc, b_solid_color);
	if ((uloc = glGetUniformLocation(program, "aspect_matrix")) > -1) glUniformMatrix4fv(uloc, 1, GL_TRUE, aspect_matrix);
}
...
void render()
{
	...
	for (auto& c : circles) {
		...
		GLint uloc;
		if ((uloc = glGetUniformLocation(program, "solid_color")) > -1) glUniform4fv(uloc, 1, c.color); // pointer version
		if ((uloc = glGetUniformLocation(program, "model_matrix")) > -1) glUniformMatrix4fv(uloc, 1, GL_TRUE, c.model_matrix);
...
void update_vertex_buffer(const std::vector<vertex>& vertices, uint N)
{
	...
	if (!(vertex_array = cg_create_vertex_array(vertex_buffer, index_buffer))) { printf("%s(): failed to create vertex aray\n", __func__); return; }
}

void update_num_of_circles(int bias)
{
	int n = num_of_circles + bias;
	if (n == num_of_circles || n  < MIN_CIRCLES || n > MAX_CIRCLES) return;

	reset_circles(circles, num_of_circles = n, t);
	printf("> number of circles = % -5d\r", num_of_circles);
}
...
void keyboard(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS) {
		...
		else if (key == GLFW_KEY_KP_ADD || (key == GLFW_KEY_EQUAL && (mods & GLFW_MOD_SHIFT))) update_num_of_circles(+1);
		else if (key == GLFW_KEY_KP_SUBTRACT || key == GLFW_KEY_MINUS) update_num_of_circles(-1);
